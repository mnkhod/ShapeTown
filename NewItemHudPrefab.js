...
export default class NewItemHudPrefab extends Phaser.GameObjects.Container {
    constructor(scene, x, y) {
        ...
        const inventory_background = scene.add.image(
            310.5,
            24.75,
            "InGameInventoryBackground"
        );
        ...
        // keyboard_1
        const keyboard_1 = this.scene.input.keyboard.addKey(
            Phaser.Input.Keyboard.KeyCodes.ONE
        );

        // keyboard_2
        const keyboard_2 = this.scene.input.keyboard.addKey(
            Phaser.Input.Keyboard.KeyCodes.TWO
        );

        // keyboard_3
        const keyboard_3 = this.scene.input.keyboard.addKey(
            Phaser.Input.Keyboard.KeyCodes.THREE
        );

        // keyboard_4
        const keyboard_4 = this.scene.input.keyboard.addKey(
            Phaser.Input.Keyboard.KeyCodes.FOUR
        );

        // keyboard_5
        const keyboard_5 = this.scene.input.keyboard.addKey(
            Phaser.Input.Keyboard.KeyCodes.FIVE
        );

        // keyboard_6
        const keyboard_6 = this.scene.input.keyboard.addKey(
            Phaser.Input.Keyboard.KeyCodes.SIX
        );

        // keyboard_7
        const keyboard_7 = this.scene.input.keyboard.addKey(
            Phaser.Input.Keyboard.KeyCodes.SEVEN
        );

        // keyboard_8
        const keyboard_8 = this.scene.input.keyboard.addKey(
        ...
        this.passiveItemSlots.forEach((slot, index) => {
            ...
            slot.on("pointerdown", () => {
                ...
                if (this.itemData[index] && this.items[index].visible) {
                    ...
                    this.cursorIcon = this.scene.add.sprite(
                        0,
                        0,
                        texture,
                        frame
    ...
    checkItem(key) {
        return (
            this.itemData.includes(key) ||
            this.mainInventoryData.some((item) => item && item.id === key)
        );
    }
    ...
    addItem(key, textureName, textureId, amount = 1, isAddable = true) {
        ...
        if (existingItemIndex !== -1 && isAddable) {
            const currentAmount = parseInt(
                this.itemCounters[existingItemIndex].text || "0"
            );
            this.itemCounters[existingItemIndex].text = (
        ...
        const existingMainIndex = this.mainInventoryData.findIndex(
    ...
    useItem(key) {
        ...
        const mainIndex = this.mainInventoryData.findIndex(
            (item) => item && item.id === key
        );
        if (mainIndex !== -1) {
    ...
    removeItemByKey(key) {
        ...
        if (index !== -1) {
            ...
            if (index === this.activeIndex) {
                ...
                const nextItemIndex = this.itemData.findIndex(
                    (item) => item !== null
                );
                if (nextItemIndex !== -1) {
