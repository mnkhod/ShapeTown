...
export default class BeachTree2Prefab extends Phaser.GameObjects.Sprite {

    constructor(scene, x, y, texture, frame) {
        super(scene, x ?? 44, y ?? 78, texture || "TreePalmSheet_02", frame ?? 0);

        this.play("BeachTree_2");
        ...
        this.treeBottom = this.y + 0;   

        scene.events.on('update', () => {
            if (!this.player) return;

            const playerNearTreeX = Math.abs(this.player.x - (this.x + this.body.offset.x - 69)) < 38;
