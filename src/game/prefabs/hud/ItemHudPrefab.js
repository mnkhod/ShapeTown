...
export default class ItemHudPrefab extends Phaser.GameObjects.Container {
    constructor(scene, x, y) {
        ...
        backpack.on(
            "pointerdown",
            () => {
                if (this.scene.reactEvent == undefined)
                    throw Error("REACT EVENT BUS NOT HOOKED IN");
    ...
    addItem(key, textureName, textureId, amount, isAddable = false) {
        if (isAddable == false) {
            let hasEmptySlot =
                this.itemData.filter((x) => x == null).length > 0
                    ? true
                    : false;
            if (!hasEmptySlot) {
        ...
        let hasItemAlready =
            this.itemData.filter((x) => x == key).length > 0 ? true : false;
        if (hasItemAlready) {
            if (isAddable == false) {
                ...
                if (amount == undefined || amount == 0) {
                    ...
                    let currAmount = parseInt(
                        this.itemCounters[itemIndex].text
                    );
                    this.itemCounters[itemIndex].text = (
                        currAmount + amount
                    ).toString();
