...
import { useQuests, useActiveQuests, useCompletedQuests, useUpdateQuestTask, useStartQuest } from '../hooks/useQuests';
...
const QuestComponentTanStack = ({ onClose }) => {
    ...
    const { data: activeQuests, isLoading: activeQuestsLoading } = useActiveQuests();
    const { data: completedQuests, isLoading: completedQuestsLoading } = useCompletedQuests();
    ...
    const handleTaskComplete = (quest, taskIndex) => {
        ...
        const isQuestStarted = activeQuests?.data?.some(activeQuest => activeQuest.id === quest.id);
    ...
    const getQuestsByTab = () => {
        switch (activeTab) {
            case 'Active':
                return activeQuests?.data || [];
            case 'Completed':
                return completedQuests?.data || [];
            case 'All':
                return allQuests?.data || [];
            default:
                return [];
        }
    };

    const isLoading = allQuestsLoading || activeQuestsLoading || completedQuestsLoading;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-gray-900 p-6 rounded-lg w-11/12 max-w-4xl max-h-5/6 overflow-auto border-2 border-yellow-500">
                ...
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-bold text-yellow-400">Quest Journal</h2>
                    <button 
                        onClick={onClose}
                ...
                {isLoading && (
                    <div className="text-center py-8">
                        <div className="text-yellow-400 text-lg">Loading quests...</div>
                    </div>
                )}
                            ...
                            >
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="text-xs text-gray-400 mt-1">
                                            ...
                                            {quest.questGiver?.name && (
                                                <span>Given by: {quest.questGiver.name}</span>
                                            )}
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        {(() => {
                                            const isActive = activeQuests?.data?.some(aq => aq.id === quest.id);
                                            const isCompleted = completedQuests?.data?.some(cq => cq.id === quest.id);
                                ...
                                {expandedQuests.has(quest.id) && (
                                    <div className="mt-4 border-t border-gray-700 pt-4">
                                        {/* Tasks */}
                                        <h4 className="text-yellow-400 font-bold mb-2">Tasks:</h4>
                                            ...
                                            {quest.tasks?.map((task, index) => (
                                                ...
                                                >
                                                    <input
                                                        ...
                                                        onChange={() => !task.completed && handleTaskComplete(quest, index)}
                                                        className="mr-2"
                                                        disabled={task.completed || updateQuestTask.isPending}
                                                    />
                                                    <span className={
                                        ...
                                        {quest.rewards?.length > 0 && (
                                            <div>
                                                <h4 className="text-yellow-400 font-bold mb-2">Rewards:</h4>
                                                <div className="text-sm text-gray-300">
                                                    {quest.rewards.map((reward, index) => (
                                                        <div key={index} className="mb-1">
                                                            ...
                                                            {reward.rewardType === 'ITEM' && reward.item && (
                                                                <span className="text-blue-300">
                                                                    📦 {reward.item.name} x{reward.itemQuantity}
                                                                </span>
                                                            )}
                                                            {reward.rewardType === 'ACHIEVEMENT' && reward.achievement && (
